...
import { Sun, Moon, Sparkles, Store, Shield } from 'lucide-react';
...
export const DeviceWrapper: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  return (
    <div className={`min-h-screen w-full transition-colors duration-300 relative ${
      ...
      <header className={`px-6 md:px-12 py-4 border-b backdrop-blur-2xl sticky top-0 z-50 flex items-center justify-between transition-colors ${
        theme === 'dark' 
          ? 'bg-[#080C14]/90 border-white/10 text-slate-100' 
          : 'bg-white/90 border-slate-200 text-slate-900 shadow-sm'
      }`}>
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-2xl bg-gradient-to-tr from-cyan-500 to-emerald-400 flex items-center justify-center text-slate-950 text-xl font-black shadow-lg shadow-cyan-500/30">
            🥛
          </div>
          <div>
            <div className="flex items-center gap-2">
              <span className="text-lg font-black tracking-tight text-transparent bg-clip-text bg-gradient-to-r from-cyan-400 via-emerald-400 to-teal-300">
                MilkTrack
              </span>
            ...
            <span className="text-xs text-slate-400 font-medium block">
              Daily Delivery Logs & Automated WhatsApp Invoicing
            </span>
          </div>
        </div>
        ...
        <div className="flex items-center gap-4">
          <div className="hidden sm:flex items-center gap-2 text-xs font-semibold text-slate-400 bg-slate-900/60 dark:bg-slate-900/80 px-3 py-1.5 rounded-full border border-slate-800">
